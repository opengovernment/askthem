...
export default async function SignInPage() {
  ...
  const hasOAuth = true; // Always true: Google (when configured) + Facebook (always shown, at least as placeholder)
        ...
        <div className="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
          ...
          {hasFacebook ? (
            ...
            >
              <button
                type="submit"
                className={`${hasGoogle ? "mt-3 " : ""}flex w-full items-center justify-center gap-3 rounded-lg border border-gray-300 bg-white px-4 py-3 text-sm font-medium text-gray-700 shadow-sm transition-colors hover:bg-gray-50`}
              >
            ...
            <button
              type="button"
              disabled
              className={`${hasGoogle ? "mt-3 " : ""}flex w-full cursor-not-allowed items-center justify-center gap-3 rounded-lg border border-gray-200 bg-gray-50 px-4 py-3 text-sm font-medium text-gray-400 shadow-sm`}
            >
          ...
          {/* Divider — only show if there's at least one OAuth provider */}
          {hasOAuth && (
