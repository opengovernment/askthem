...
describe QuestionObserver do
  before :each do
    Metadatum.with(session: 'openstates').create(id: 'zz', abbreviation: 'zz')
    @bill = Bill.with(session: 'openstates').create(state: 'zz', question_count: 5)
    @person = Person.with(session: 'openstates').create(state: 'zz', question_count: 5)
  end

  let :question do
    FactoryGirl.create(:question, person: @person, bill: @bill)
  end

  let :answered_question do
    FactoryGirl.create(:question, person: @person, bill: @bill, answered: true)
  end

  context '#after_create' do
    it "should increment the person's question count" do
      expect{FactoryGirl.create(:question, person: @person)}.to change{@person.question_count}.by(1)
    end

    it "should increment the bill's question count" do
      expect{FactoryGirl.create(:question, bill: @bill)}.to change{@bill.question_count}.by(1)
    end
  end
